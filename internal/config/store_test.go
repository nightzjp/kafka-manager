...
func TestEncryptDecrypt(t *testing.T) {
	...
	if !strings.HasPrefix(ciphertext, "enc:v1:") || strings.Contains(ciphertext, "kafka-secret") {
	...
	if _, err := Decrypt([]byte("abcdef0123456789abcdef0123456789"), ciphertext); err == nil {
...
func TestStoreSaveCreatesBackupAndReloads(t *testing.T) {
	...
	if _, err := store.Save([]byte(updated)); err != nil {
		t.Fatalf("Save() error = %v", err)
	}
	loaded, err := store.Load()
	if err != nil || loaded.Clusters[0].Brokers[0] != "localhost:9093" {
...
func TestRuntimeDecryptsKafkaPasswords(t *testing.T) {
	key := []byte("0123456789abcdef0123456789abcdef")
	encrypted, err := Encrypt(key, "secret")
	if err != nil {
		t.Fatal(err)
	}
	cfg := Config{Clusters: []ClusterConfig{{ID: "test", Security: SecurityConfig{Password: encrypted}}}}
	runtime, err := Runtime(cfg, key)
	if err != nil {
		t.Fatal(err)
	}
	if runtime.Clusters[0].Security.Password != "secret" {
		t.Fatalf("password=%q", runtime.Clusters[0].Security.Password)
	}
	if cfg.Clusters[0].Security.Password != encrypted {
