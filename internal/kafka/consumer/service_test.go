...
func TestListSortsGroupsByLag(t *testing.T) {
	...
	if err != nil || len(groups) != 2 || groups[0].Name != "large" {
		t.Fatalf("List() = %+v, %v", groups, err)
	}
}

func TestPartitionLagJSONUsesDistinctFields(t *testing.T) {
	data, err := json.Marshal(PartitionLag{Topic: "orders", Partition: 1, CurrentOffset: 2, EndOffset: 5, Lag: 3})
	if err != nil {
		t.Fatal(err)
	}
	for _, field := range []string{`"currentOffset":2`, `"endOffset":5`, `"lag":3`} {
...
func TestResetRejectsUnsafeRequests(t *testing.T) {
	...
	for _, request := range []ResetRequest{{Group: "", Mode: "latest"}, {Group: "orders", Mode: "unknown"}, {Group: "orders", Mode: "absolute", Offset: -1}} {
		if err := service.Reset(context.Background(), request); err == nil {
