...
- 首页由后端定时采样，各浏览器共享内存中的短期历史结果，不引入指标数据库。多个集群使用有界并发采集，同一轮采样不会重叠；离线集群采用最长 5 分钟的指数退避。
- Dashboard API 只复制内存快照，不在 HTTP 请求链路中访问 Kafka，因此浏览器数量不会放大 Kafka 管理请求。没有浏览器打开时，后端仍按配置持续采样。
- 集群管理器维护权威连接配置，过期的后台重连不能覆盖新配置或重新创建已删除集群。热更新只重连 Broker 或认证发生变化的集群，其他健康连接保持不动。
