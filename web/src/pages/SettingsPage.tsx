...
export function SettingsPage({ onSaved }: { onSaved: () => void }) {
  ...
  return <>
    ...
    <section className="retention"><h3>后台监控采样</h3><label>采样间隔（秒）<input type="number" min="5" max="3600" value={cfg.dashboard.sampleIntervalSeconds} onChange={(event) => setCfg({ ...cfg, dashboard: { ...cfg.dashboard, sampleIntervalSeconds: Number(event.target.value) } })} /></label><label>内存历史点数<input type="number" min="2" max="10000" value={cfg.dashboard.historyPoints} onChange={(event) => setCfg({ ...cfg, dashboard: { ...cfg.dashboard, historyPoints: Number(event.target.value) } })} /></label></section>
