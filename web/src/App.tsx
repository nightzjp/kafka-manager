...
export function App(){
 const [authenticated,setAuthenticated]=useState<boolean|null>(null);const[page,setPage]=useState<Page>('dashboard');const[clusters,setClusters]=useState<Cluster[]>([]);const[history,setHistory]=useState<Record<string,DashboardPoint[]>>({});const[clusterId,setClusterId]=useState('');
 const loadClusters=()=>api.get<{items:Cluster[];history:Record<string,DashboardPoint[]>}>('/api/v1/dashboard').then(({items,history})=>{setClusters(items);setHistory(history||{});setClusterId(current=>current||items[0]?.id||'')});
